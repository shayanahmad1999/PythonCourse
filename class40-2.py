...
# Create combined morning inventory
department_store = {}

for shop in stores:
    for item, price in shop.items():
        if item != "name":
            department_store[item] = price

print("===== Morning Inventory =====")

for item, price in sorted(department_store.items()):
...
# Create purse with 1000 gold pieces
purse = 1000
...
for shop in stores:
    ...
    buy_item = input(
        "What do you want to buy? Type 'exit' to skip this shop: "
    ).strip().lower()

    # Skip the current shop
    if buy_item == "exit":
        print(f"You left {shop['name']} without buying anything.")
        continue
    ...
    item_price = shop[buy_item]

    # Check whether the player can afford the item
    if item_price > purse:
        print(f"You do not have enough gold to buy {buy_item}.")
        continue
    ...
    print(f"You purchased {buy_item.title()} for {item_price} Gp.")
    print(f"Gold remaining: {purse} Gp")
...
# Create combined evening inventory
department_store_after = {}

for shop in stores:
    for item, price in shop.items():
        if item != "name":
            department_store_after[item] = price

print("\n===== Evening Inventory =====")

for item, price in sorted(department_store_after.items()):
    print(f"{item.title()}: {price} Gp")